...
(function ($, _) {
	...
	SirTrevor.toMarkdown = function (content, type) {
		...
		markdown = markdown.replace(/( class=(")?Mso[a-zA-Z]+(")?)/g, '')
			.replace(/<!--(.*?)-->/g, '')
			.replace(/\/\*(.*?)\*\//g, '')
			.replace(/<(\/)*(meta|link|\\?xml:|st1:|o:|font)(.*?)>/gi, '')
			.replace(/<span>(.*)<\/span>/, "$1")
		...
		// Strip remaining HTML, minus allowed literals
		markdown = markdown.replace(/<\/?(?!\/?strike|\/?del|\/?code|\/?s>|\/?u)[^>]+(>|$)/g, "");
	...
	(function () {
		...
		var Code = SirTrevor.Formatter.extend({
			...
			onClick: function () {
				if (this.isActive()){
					...
					container.outerText = container.innerText;
				}
				else{
					var sel = window.getSelection();
			...
			isActive: function () {
				var sel = window.getSelection();
				var container = sel.getRangeAt(0).startContainer;
				if (container.tagName == "CODE" || container.parentNode.tagName == "CODE"){
					return true;
				}
				else{
					return false;
		...
		SirTrevor.Formatters.Code = new Code();
	...
	SirTrevor.Editor = (function () {
		...
		_.extend(SirTrevorEditor.prototype, FunctionBind, SirTrevor.Events, {
			...
			onBlockDropped: function (block_id) {
				...
				if (!_.isUndefined(block) && !_.isEmpty(block.getData()) &&
